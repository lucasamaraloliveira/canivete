...
import { X, Heart, MessageSquarePlus, RefreshCw, Check, ShieldAlert, Mail, Copy } from 'lucide-react';
                    ...
                    >
                        ...
                        <div className="absolute top-4 right-4 sm:top-6 sm:right-6 z-10">
                            <button
                                ...
                                className="p-2 sm:p-3 hover:bg-text-main/10 rounded-2xl transition-all text-text-main/70 hover:text-text-main"
                            >
                                <X size={20} />
                            </button>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar">
                            <div className="grid grid-cols-1 md:grid-cols-2 h-auto md:h-full">
                                {/* Coluna de Doação */}
                                <div className="p-6 sm:p-8 lg:p-10 border-b md:border-b-0 md:border-r border-white/5 flex flex-col items-center text-center">
                                    <div className="w-12 h-12 sm:w-14 sm:h-14 bg-red-500/20 text-red-500 rounded-2xl sm:rounded-[24px] flex items-center justify-center mb-3 sm:mb-4 shadow-[0_0_30px_-5px_rgba(239,68,68,0.4)]">
                                        <Heart size={isMobile ? 24 : 28} className="fill-current" />
                                    </div>
                                    ...
                                    <div className="w-full space-y-4 sm:y-5 min-h-[240px] flex flex-col items-center justify-center">
                                        {selectedBank ? (
                                            <div className="flex flex-col items-center gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                                <div className="relative mx-auto w-40 h-40 sm:w-48 sm:h-48 bg-white p-3 rounded-[32px] shadow-2xl flex items-center justify-center">
                                                    <QRCodeSVG value={pixPayload} size={isMobile ? 140 : 160} level="H" />
                                                </div>
                                            ...
                                            <div className="flex flex-col items-center gap-4">
                                                <div className="mx-auto w-36 h-36 sm:w-44 sm:h-44 bg-white p-2.5 sm:p-3 rounded-2xl sm:rounded-3xl shadow-lg flex items-center justify-center transition-transform hover:scale-105">
                                        ...
                                        <div className="w-full space-y-2 pt-4">
                                            ...
                                            <div className="relative flex items-center gap-2 bg-text-main/5 border border-white/5 rounded-2xl p-1 pr-2">
                                                ...
                                                <button
                                                    ...
                                                    className={cn(
                                                        "px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl font-bold text-[9px] sm:text-[10px] uppercase tracking-widest transition-all active:scale-95 flex items-center gap-1.5 sm:gap-2 shrink-0 text-white",
                                                        pixCopied ? "bg-green-500" : "bg-text-main text-bg-main hover:opacity-90 shadow-lg"
                                                    )}
                                                >
                                ...
                                {/* Coluna de Sugestão */}
                                <div className="p-6 sm:p-8 lg:p-10 flex flex-col bg-text-main/[0.02]">
                                    <div className="w-12 h-12 sm:w-14 sm:h-14 bg-blue-500/20 text-blue-500 rounded-2xl sm:rounded-[24px] flex items-center justify-center mb-3 sm:mb-4 shadow-[0_0_30px_-5px_rgba(59,130,246,0.4)]">
                                    ...
                                    <div className="flex-1 flex flex-col gap-3 sm:gap-4">
                                        ...
                                        <button
                                            ...
                                            className={cn(
                                                ...
                                                suggestionStatus === 'success' ? "bg-green-500 text-white border-green-400" :
                                                    suggestionStatus === 'error' ? "bg-red-500 text-white border-red-400" :
                                                        "bg-text-main text-bg-main hover:opacity-90 border-transparent shadow-[0_10px_20px_-5px_rgba(0,0,0,0.3)]"
                                            )}
                                        >
                                            {isSendingSuggestion ? <><RefreshCw size={16} className="animate-spin" /> Processando...</> :
                                                suggestionStatus === 'success' ? <><Check size={16} /> Enviado com Sucesso!</> :
                                                    suggestionStatus === 'error' ? <><ShieldAlert size={16} /> Houve um Erro</> :
                                                        <><Mail size={16} /> Enviar Sugestão Agora</>}
                                        </button>
                                    </div>
                                </div>
                            </div>
